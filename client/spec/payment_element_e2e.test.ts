...
test.beforeEach(async ({ page }) => {
  ...
  const firstShirtAddButton = await page.locator('[data-test="add-to-cart"]').first();
...
test('Adding an item to your cart populates it in the product card and navbar badges', async ({
  ...
  const productCardBadge = await page.locator('[data-test="product-card-badge"]').first();
...
test('Removing an item to your cart removes it from the product card and navbar badges', async ({
  ...
  const firstShirtRemoveButton = await page.locator('[data-test="remove-from-cart"]').first();
